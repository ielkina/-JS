...
console.log(getOwnDivisors(220));//[2,  4,  5, 10,  11, 20, 22, 44, 55, 110 ]
console.log(getOwnDivisors(284));//[ 2, 4, 71, 142 ]
...
function FriendNum(num) {//находит дружественные числа
	let a = arrSum(num);
	let b = arrSum(a);
	if (b == num && num != a) {
		return a;
...
function arrSum(num) { //суммирует элементов массива(числа)
	let arr = [];
	let sum = 0;
	for (let i = 1; i < num; i++) {
		if (num % i == 0) {
